#!/usr/bin/env python3
"""agent-vitals hook: doctor, at the moment an MCP server is added.
...
def explain(r: dict) -> str:
    ...
    if dep:
        parts.append(f"package deprecated: {str(dep)[:160]}")
    lead = ", ".join(parts)
...
def main() -> int:
    ...
    text = " ".join(explain(r) for r in found) + (
        " These are dates and registry flags from agent-vitals, not a verdict on the code; a finished tool can go"
