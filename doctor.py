#!/usr/bin/env python3
"""agent-vitals doctor: the census, pointed at your own machine.

The daily census measures the ecosystem. This measures the part of it you
actually run. It reads the MCP server entries in the config files of the
...
VERSION = "0.1.0"
UA = "agent-vitals-doctor (+https://github.com/Keremozdemirra/agent-vitals)"
...
class Net:
    ...
    def from_census(self, repo: str) -> dict:
        if self.census is None:
            ...
            try:
                data = json.loads(local.read_text()) if local.is_file() else self.get(CENSUS)
            except (OSError, json.JSONDecodeError):
                data = {}
            self.census = {r["full_name"].lower(): r for r in (data.get("repositories") or [])}
...
def render_text(results: list[dict], searched: list[str]) -> str:
    ...
    fmt = lambda cells: "  ".join(c[:w].ljust(w) for c, w in zip(cells, widths)).rstrip()
...
def render_markdown(results: list[dict], today: dt.date) -> str:
    ...
    out += ["", summary_line(results), "",
            "_Checked with [agent-vitals doctor](https://github.com/Keremozdemirra/agent-vitals#check-your-own-setup). "
